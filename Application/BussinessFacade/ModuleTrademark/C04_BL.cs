...
{
    public class App_Detail_C04_BL
    {
        public decimal Insert(App_Detail_C04_Info pInfo)
        {
            try
            {
                App_Detail_C04_DA objData = new App_Detail_C04_DA();
        ...
        public App_Detail_C04_Info GetByID(decimal p_app_header_id, string p_language_code,
            ...
            ref List<AppDocumentOthersInfo> _LstDocumentOthersInfo)
        {
            try
            {
                var objData = new App_Detail_C04_DA();
                DataSet dataSet = objData.GetByID(p_app_header_id, p_language_code);
                App_Detail_C04_Info app_Detail_C04 = CBO<App_Detail_C04_Info>.FillObjectFromDataSet(dataSet);
                ...
                return app_Detail_C04;
            }
            catch (Exception ex)
            {
                Logger.LogException(ex);
                return new App_Detail_C04_Info();
            }
        }
    }
}
