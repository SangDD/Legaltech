...
-- Generated 10-Oct-2018 21:39:36 from LEGALTECH@LEGALONLINE
...
-- Generated 10-Oct-2018 21:39:37 from LEGALTECH@LEGALONLINE
...
BEGIN
    ...
    SELECT * FROM VIEW_APP_HEADER WHERE case_code = v_case_code;
...
BEGIN
    ...
    --LAY DANH SACH CAC TAI LIEU DINH KEM
    OPEN p_cursor_doc FOR
    SELECT * FROM APP_DOCUMENT A  WHERE app_header_id = p_app_header_id AND  language_code = p_language_code ORDER BY id;
...
-- Generated 10-Oct-2018 21:39:38 from LEGALTECH@LEGALONLINE
...
BEGIN
    ...
    --SELECT appcode INTO v_appcode FROM application_header WHERE id = P_APP_HEADER_ID;

    SELECT COUNT(*) INTO v_count FROM sys_app_fix_charge WHERE appcode = p_case_code AND fee_id = P_FEE_ID;
    IF v_count = 0 THEN
        v_amount_usd := 0;
    ELSE
        SELECT amount_usd INTO v_amount_usd FROM sys_app_fix_charge WHERE appcode = p_case_code AND fee_id = P_FEE_ID;
...
BEGIN
    ...
    OPEN  P_CURSOR FOR
    SELECT A.* , B.* FROM APPLICATION_HEADER A
    LEFT JOIN APP_DETAIL_04NH B ON A.ID = B.APP_HEADER_ID
    WHERE   A.LANGUAGUE_CODE =P_LANGUAGE   AND  APP_HEADER_ID = P_APP_HEADER_ID  AND STATUS = P_STATUS AND DELETED =0 ;
...
PROCEDURE PROC_GET_WARING_DATA
(
...
BEGIN
    ...
    (
        ...
        LANGUAGUE_CODE ,ADDRESS ,DATENO,MONTHS,YEARS,Case_code,CLIENT_REFERENCE,CASE_NAME,STATUS_DATE, DDSHCN , MADDSHCN
    )
    VALUES
    (
    ...
    IF P_STATUS = pkg_common.APP_STATUS_SEND_WAIT_GRANT THEN
...
BEGIN
    ...
    SELECT case_code, status INTO v_case_code, v_status
...
IS
BEGIN
    OPEN p_cursor FOR SELECT * FROM view_app_header WHERE upper(app_no) = upper(p_appNo);
EXCEPTION
WHEN OTHERS THEN
    RAISE;
END ;
...
BEGIN
    ...
    FROM view_app_header WHERE upper(case_code) = upper(p_case_code) AND LANGUAGUE_CODE = p_language_code;

    OPEN p_cursor FOR SELECT * FROM view_app_header WHERE upper(case_code) = upper(p_case_code) AND LANGUAGUE_CODE = p_language_code;
...
IS
    V_SQL VARCHAR2(4000) DEFAULT 'SELECT ';
BEGIN

    IF P_USER_TYPE = 1 THEN
        -- ADMIN THI LAY TAT
        V_SQL := V_SQL || '
        SUM(CASE WHEN A.STATUS = 1 THEN 1 ELSE 0 END) AS W_LUUTAM,
        SUM(CASE WHEN A.STATUS = 2 THEN 1 ELSE 0 END) AS W_CHOPHANLOAI,
        SUM(CASE WHEN A.STATUS = 3 THEN 1 ELSE 0 END) AS W_PHANCHOLUATSU,
        SUM(CASE WHEN A.STATUS = 4 THEN 1 ELSE 0 END) AS W_LUATSUDACOMFIRM,
        SUM(CASE WHEN A.STATUS = 5 THEN 1 ELSE 0 END) AS W_CHOKHACHHANGCOMFIRM,
        SUM(CASE WHEN A.STATUS = 6 THEN 1 ELSE 0 END) AS W_KHACHHANGDACOMFIRM,
        SUM(CASE WHEN A.STATUS = 7 THEN 1 ELSE 0 END) AS W_DAGUILENCUC ';
    ELSIF P_USER_TYPE = 2 THEN
        -- LUAT SU
        V_SQL := V_SQL || '
        SUM(CASE WHEN A.STATUS = 1 THEN 1 ELSE 0 END) AS W_LUUTAM,
        SUM(CASE WHEN A.STATUS = 2 THEN 1 ELSE 0 END) AS W_CHOPHANLOAI,
    ...
    V_SQL := V_SQL || ' FROM   APPLICATION_HEADER A WHERE DELETED= 0 ';
    OPEN P_CURSOR FOR V_SQL;
...
BEGIN
    ...
    UPDATE APPLICATION_HEADER
    SET status = pkg_app_header.APP_HEADER_STATUS_DAGUI2LS,
    ...
    -- Update trang thai review
    --SELECT case_code INTO v_case_code FROM application_header WHERE id = p_application_header_id AND LANGUAGUE_CODE = p_language_code;
...
APP_STATUS_SEND_WAIT_GRANT CONSTANT NUMBER(1) := 2;       -- da gui cho phan loai
APP_STATUS_GRANT_LAWER CONSTANT NUMBER(1) := 3;           -- da phan loai, cho LS phan hoi
APP_STATUS_LAWER_CONFIRMED CONSTANT NUMBER(1) := 4;       -- LS DA CONFIRM
APP_STATUS_WAIT_KH_CONFIRM CONSTANT NUMBER(1) := 5;       -- CHO KHACH HANG CONFIRM
APP_STATUS_KH_CONFIRMED CONSTANT NUMBER(1) := 6;          -- KHACH HANG DA CONFIRM
APP_STATUS_KH_REJECT CONSTANT NUMBER(1) := 7;             -- KHACH HANG TU CHOI
APP_STATUS_GRANT_EMPLOYEE CONSTANT NUMBER(1) := 8;        -- DA PHAN LOAI CHO NHAN VIEN
APP_STATUS_DANOPDON CONSTANT NUMBER(1) := 9;              -- NHAN VIEN DA NOP DON
APP_STATUS_ADVICE_FILING CONSTANT NUMBER(2) := 10;        -- ADVICE FILING XONG
...
BILLING_DETAIL_TYPE_OTHER CONSTANT NUMBER(1) := 4;
...
BEGIN
    ...
    OPEN p_cursor_detail FOR
        ...
        WHERE billing_id = p_billing_id ORDER BY BILLING_DETAIL_ID;

    OPEN p_cursor_header FOR
        SELECT * FROM view_app_header
        WHERE case_code = p_app_case_code AND languague_code = p_language_code;
...
IS
    ...
    v_billing_id NUMBER;
BEGIN
    SELECT app_case_code,billing_id INTO v_app_case_code,v_billing_id FROM billing_header WHERE CASE_CODE = p_case_code AND language_code = p_language_code;
    ...
    OPEN p_cursor_detail FOR
        SELECT * FROM billing_detail
        WHERE billing_id = v_billing_id ORDER BY BILLING_DETAIL_ID;

    OPEN p_cursor_header FOR
        SELECT * FROM view_app_header
        WHERE case_code = v_app_case_code AND languague_code = p_language_code;
...
BEGIN
    ...
    (
        ...
        total_pre_tex, tex_fee, total_amount, currency, currency_rate, created_by, created_date, language_code

    )
    VALUES
    (
        v_id, V_CASE_CODE, p_billing_type, p_app_case_code, p_billing_date, p_deadline, p_request_by, p_approve_by, p_status,
        p_total_pre_tex, p_tex_fee, p_total_amount, p_currency, p_currency_rate, p_created_by, p_created_date, p_language_code
    );
...
IS
    ...
    --v_Content VARCHAR(2000) DEFAULT '';
...
V_CONDITION VARCHAR2(2000) ;
 ...
 V_SQL:='SELECT  A.APPCODE, A.MASTER_NAME ,A.MASTER_ADDRESS,A.MASTER_PHONE,A.MASTER_EMAIL,A.MASTER_FAX,
            ...
            B.CDK_NAME_4,B.CDK_ADDRESS_4,B.CDK_PHONE_4,B.CDK_FAX_4,B.CDK_EMAIL_4
...
BEGIN
 ...
 FOR ITEM IN V_CUR_CONDITION LOOP
    ...
    --pkg_log.LOGMSG(V_TOTAL_COUNT); COMMIT;
    EXECUTE IMMEDIATE V_TOTAL_COUNT INTO V_TOTAL_COUNT;
...
-- Generated 10-Oct-2018 21:39:39 from LEGALTECH@LEGALONLINE
...
-- mac dinh them moi trang thai cua nó la moi tao chua active
...
BEGIN
    ...
    END LOOP;
     ...
     V_CONDITION :=  V_CONDITION || ' AND UPPER(A.PROCESSOR_BY) = ''' || V_USERNAME || ''' ';

    V_SQL := 'SELECT  A.* FROM VIEW_REMIND A WHERE A.STATUS = 1 ';
    V_SQL :=  V_SQL || V_CONDITION;
    ...
    IF(V_EXISTED = 1) THEN
        ...
        IF p_type = pkg_common.USER_TYPE_LAWER THEN
            UPDATE s_users SET
                ...
                hourly_rate_usd = p_hourly_rate_usd
            WHERE id = p_userId;
...
(
    ...
    P_SEARCH_VALUE IN SEARCH_DETAIL.SEARCH_VALUE % TYPE,
    P_SEARCH_OPERATOR IN SEARCH_DETAIL.SEARCH_OPERATOR % TYPE,
    P_ANDOR IN SEARCH_DETAIL.ANDOR % TYPE,
    P_RETURN OUT NUMBER
);
...
P_RETURN := 0;
 ...
 V_CASE_CODE := 'SEARCH' || to_char(SYSDATE,'yyyyMMdd') || V_ID_CASE_CODE;
    ...
    (
        ...
        STATUS, LAWER_ID, CREATED_BY, CREATED_DATE,  LANGUAGE_CODE
     )
    VALUES
    (
        V_ID, V_CASE_CODE, P_CLIENT_REFERENCE, P_CASE_NAME, TRUNC(P_REQUEST_DATE), TRUNC(P_RESPONSE_DATE),
        P_STATUS, P_LAWER_ID, P_CREATED_BY, TRUNC(P_CREATED_DATE),  P_LANGUAGE_CODE
    );
...
IS
 BEGIN
    OPEN P_CURSOR FOR
    SELECT A.SEARCH_ID, A.CASE_CODE, A.CLIENT_REFERENCE, A.CASE_NAME,
        A.REQUEST_DATE, A.RESPONSE_DATE, A.STATUS, A.LAWER_ID,
        A.CREATED_BY, A.CREATED_DATE, A.MODIFIED_BY, A.MODIFIED_DATE,
        A.LANGUAGE_CODE, B.CONTENT
    FROM SEARCH_HEADER A
    LEFT JOIN SEARCH_QUESTION B ON A.SEARCH_ID = B.SEARCH_ID
    LEFT JOIN S_USERS US ON A.CREATED_BY  = US.USERNAME AND  US.DELETED = 0
    WHERE A.DELETED = 0 AND A.SEARCH_ID = P_SEARCH_ID;
    ...
    (
        ...
        (
            SELECT A.*,  A1.CONTENT AS SEARCH_TYPE_NAME,  A2.CONTENT AS ANDOR_NAME, A3.CONTENT  AS SEARCH_OPERATOR_NAME
            FROM SEARCH_DETAIL A
            LEFT JOIN ALLCODE A1 ON A.SEARCH_TYPE = A1.CDVAL AND A1.CDNAME = 'SEARCH_OBJECT' AND A1.CDTYPE =  'SEARCHTYPE'
            LEFT JOIN  ALLCODE A2 ON A.ANDOR = A2.CDVAL AND A2.CDNAME = 'SEARCH_OBJECT' AND A2.CDTYPE =  'ANDOR'
            LEFT JOIN  ALLCODE A3 ON A.SEARCH_OPERATOR = A3.CDVAL AND A3.CDNAME = 'SEARCH_OBJECT' AND A3.CDTYPE =  'OPERATOR'
...
V_SEARCHID NUMBER;
 ...
 SELECT SEARCH_ID INTO V_SEARCHID FROM  SEARCH_HEADER WHERE UPPER(CASE_CODE) = UPPER(P_CASECODE) AND DELETED = 0 ;
    ...
    LEFT JOIN SEARCH_QUESTION B ON A.SEARCH_ID = B.SEARCH_ID
    LEFT JOIN S_USERS US ON A.CREATED_BY  = US.USERNAME AND  US.DELETED = 0
    WHERE A.DELETED = 0 AND A.SEARCH_ID = V_SEARCHID;
...
BEGIN
    ...
    P_RETURN := 0;
    UPDATE SEARCH_HEADER A SET A.DELETED = 1 WHERE A.SEARCH_ID = P_SEARCH_ID;
...
BEGIN
    ...
    (
        ID,  SEARCH_ID, SEARCH_TYPE, SEARCH_VALUE, SEARCH_OPERATOR, ANDOR
    )
    VALUES
    (
        V_ID, P_SEARCH_ID,  P_SEARCH_TYPE, P_SEARCH_VALUE, P_SEARCH_OPERATOR, P_ANDOR
    );
P_RETURN := V_ID;
...
BEGIN
    ...
    (
        id,case_code,name, app_case_code, lawer_id, time_date,from_time,to_time, hours,hours_adjust, notes, status, created_by, created_date

    )
    VALUES
    (
        v_id,V_CASE_CODE,p_name, p_app_case_code, p_lawer_id, p_time_date,p_from_time,p_to_time, p_hours,p_hours_adjust, p_notes, p_status, p_created_by, p_created_date
    );
...
BEGIN
    ...
    IF V_APP_CODE <> 'ALL' THEN
        V_CONDITION := V_CONDITION || ' AND T.app_code = ''' || V_APP_CODE || '''';
    END IF;
...
BEGIN
    ...
    V_SQL := 'SELECT  A.* FROM VIEW_TODOS A WHERE 1 = 1 ';
    V_SQL :=  V_SQL || V_CONDITION;
...
BEGIN
    ...
    IF p_type = pkg_common.TODO_TYPE_APP THEN
        ...
        -- lay trang thai cua app
        SELECT a.id,a.status,created_by,a.modify_by,s.username employee
        INTO v_app_id,v_status_app,v_created_by,v_modify_by,v_employee
        FROM application_header a
        LEFT JOIN s_users s ON s.id = a.employee
        WHERE case_code = p_case_code AND LANGUAGUE_CODE = p_language_code;

        -- lay user admin
        -- SELECT created_by INTO v_user_admin FROM app_lawer WHERE application_header_id = v_app_id;

        -- lay content
        IF p_language_code = pkg_common.VI_VN THEN
            SELECT content INTO v_Content FROM allcode WHERE cdname = 'APP' and cdtype = 'STATUS' AND cdval = v_status_app;
            v_Content := v_Content;
        ELSE
            SELECT content_eng INTO v_Content FROM allcode WHERE cdname = 'APP' and cdtype = 'STATUS' AND  cdval = v_status_app;
            v_Content := v_Content;
        END IF;

        -- khach hang da gui cho admin phan cong
        IF v_status_app = pkg_common.APP_STATUS_SEND_WAIT_GRANT THEN
        ...
        -- modify_by/lawer
        ELSIF v_status_app = pkg_common.APP_STATUS_GRANT_LAWER OR v_status_app = pkg_common.APP_STATUS_DANOPDON THEN
            ...
            -- thang yc là admin
            v_request_by := v_employee; -- v_user_admin
        ...
        ELSIF v_status_app = pkg_common.APP_STATUS_LAWER_CONFIRMED OR v_status_app = pkg_common.APP_STATUS_KH_CONFIRMED OR v_status_app = pkg_common.APP_STATUS_KH_REJECT THEN
            ...
            -- thang yc là thang update trang thai
        ...
        ELSIF v_status_app = pkg_common.APP_STATUS_WAIT_KH_CONFIRM OR v_status_app = pkg_common.APP_STATUS_ADVICE_FILING THEN
    ...
    ELSIF p_type = pkg_common.TODO_TYPE_BILLING THEN
        ...
        -- lay user admin
        SELECT a.created_by,u.username INTO v_user_admin, v_lawer
        FROM app_lawer a
        JOIN s_users u ON u.id = a.lawer_id
        ...
        ELSIF v_status_billing = pkg_common.BILLING_H_STATUS_APPROVE THEN
            ...
            -- ng xu ly la thang lawer va khach hang
            v_processor_by := v_lawer;
            v_processor_by_2 := v_lawer;
    ...
    ELSIF p_type = pkg_common.TODO_TYPE_TIMESHEET THEN
        ...
        SELECT status,app_case_code into v_status_timesheet,v_app_case_code FROM timesheet WHERE case_code = p_case_code;
        ...
        -- lay content
        IF p_language_code = pkg_common.VI_VN THEN
            SELECT content INTO v_Content FROM allcode WHERE cdname = 'TIMESHEET' and cdtype = 'STATUS' AND cdval = v_status_timesheet;
            v_Content := v_Content;
        ELSE
            SELECT content_eng INTO v_Content FROM allcode WHERE cdname = 'TIMESHEET' and cdtype = 'STATUS' AND cdval = v_status_timesheet;
            v_Content := v_Content;
        END IF;
    ...
    ELSIF p_type = pkg_common.TODO_TYPE_SEARCH THEN
        ...
        SELECT status,lawer_name,admin_name, created_by into v_status_search,v_lawer,v_user_admin, v_created_by
        FROM view_search WHERE case_code = p_case_code;

        -- lay content
        IF p_language_code = pkg_common.VI_VN THEN
            SELECT content INTO v_Content FROM allcode WHERE cdname = 'SEARCH_OBJECT' and cdtype = 'STATUS' AND cdval = v_status_search;
            v_Content := v_Content;
        ELSE
            SELECT content_eng INTO v_Content FROM allcode WHERE cdname = 'SEARCH_OBJECT' and cdtype = 'STATUS' AND cdval = v_status_search;
            v_Content := v_Content;
        END IF;
...
BEGIN
    ...
    END IF;
     ...
     IF V_TYPE =  'ORDER' THEN
        V_CONDITION :=  V_CONDITION || ' AND UPPER(A.REQUEST_BY) = ''' || V_USERNAME || ''' ';
    END IF;
    ...
    OPEN P_CURSOR_TODO FOR SELECT
        SUM(CASE WHEN  A.TYPE = 1 THEN 1 ELSE 0 END) AS NUMBER_APPS,
        SUM(CASE WHEN A.TYPE = 2 THEN 1 ELSE 0 END) AS NUMBER_QUETIONS,
        SUM(CASE WHEN A.TYPE = 3 THEN 1 ELSE 0 END) AS NUMBER_SEARCH,
        SUM(CASE WHEN A.TYPE = 4 THEN 1 ELSE 0 END) AS NUMBER_TIMESHEET ,
        SUM(CASE WHEN A.TYPE = 5 THEN 1 ELSE 0 END) AS NUMBER_BILLS
     FROM b_todos A WHERE UPPER(A.processor_by) =  UPPER(V_USERNAME) AND A.STATUS = 0;
