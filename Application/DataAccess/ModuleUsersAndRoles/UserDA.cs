		...
		public static DataSet FindUser(string keysSearch, OptionFilter options, ref int totalRecord)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(keysSearch))
                {
                    ...
                    if (arrKeySearch.Length == 8)
                    {
                        userName = arrKeySearch[0];
	                    fullName = arrKeySearch[1];
						groupId = KeySearch.FilterComboboxValue(arrKeySearch[2]);
                        status = KeySearch.FilterComboboxValue(arrKeySearch[3]);
	                    positionId = KeySearch.FilterComboboxValue(arrKeySearch[4]);
	                    departmentId = KeySearch.FilterComboboxValue(arrKeySearch[5]);
	                    branchId = KeySearch.FilterComboboxValue(arrKeySearch[6]);
	                    wareHouseId = KeySearch.FilterComboboxValue(arrKeySearch[7]);
                    }
                }
        ...
        public static int AddUser(UserInfo userAdd, string arrGroupId)
        {
            try
            {
	            ...
	            OracleHelper.ExecuteDataset(
		            ...
		            new OracleParameter("p_CanChangeInstance", OracleDbType.Varchar2, userAdd.CanChangeInstanceWhenOutStock, ParameterDirection.Input),
        ...
        public static int EditUser(UserInfo userEdit, string arrGroupId)
        {
            try
            {
	            ...
	            OracleHelper.ExecuteDataset(
		            ...
		            new OracleParameter("p_CanChangeInstance", OracleDbType.Varchar2, userEdit.CanChangeInstanceWhenOutStock, ParameterDirection.Input),
