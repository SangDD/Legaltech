    ...
    public class OracleHelper
    {
        ...
        public static DataSet ExecuteDataset(string connectionString, CommandType commandType, string commandText, params OracleParameter[] commandParameters)
        {
            foreach (OracleParameter item in commandParameters)
            {
                ...
                if (item.OracleDbType != OracleDbType.Varchar2 && item.OracleDbType != OracleDbType.NVarchar2)
                {
                    continue;
                }
                ...
                {
                    foreach (string _injection in Common.Common.c_dic_Injection.Keys)
                    {
                        if (item.Value.ToString().ToUpper().Contains(_injection))
                        {
                            return null;
                        }
                    }
