...
(function () {
    ...
    DateFormatter.prototype = {
        ...
        parseDate: function (vDate, vFormat) {
            ...
            vDateParts = vDate.replace(self.separators, '\0').replace(/\0{2,}/g, '\0').split('\0');
                    ...
                    input.on('keydown.xdsoft', function (event) {
                        ...
                        if (((key >= KEY0 && key <= KEY9) || (key >= _KEY0 && key <= _KEY9)) || (key === BACKSPACE || key === DEL)) {
                            ...
                            if ($selectionLength == 0) {
                                ...
                                var $strReplace = digit;
                                for (var i = 1; i < $selectionLength; i++) {
                                    if (/[0-9_]/.test(options.mask.substr(pos + i, 1)) == false) {
                                        $strReplace += options.mask.substr(pos + i, 1);
                                    }
                                    else {
                                        $strReplace += '_';
                                    }
                                }
                                val = val.substr(0, pos) + $strReplace + val.substr(pos + $selectionLength);
                                pos += (key === BACKSPACE || key === DEL) ? 0 : 1;
                            }
