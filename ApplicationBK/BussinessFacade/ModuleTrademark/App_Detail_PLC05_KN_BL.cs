...
{
    public class App_Detail_PLC05_KN_BL
    {
        ...
        public decimal Insert(App_Detail_PLC05_KN_Info pInfo)
        {
            try
            {
                App_Detail_PLC05_KN_DA objData = new App_Detail_PLC05_KN_DA();
        ...
        public App_Detail_PLC05_KN_Info GetByID(decimal p_app_header_id, string p_language_code, 
            ...
            ref List<AppDocumentInfo> appDocumentInfos, ref List<AppFeeFixInfo> appFeeFixInfos)
        {
            try
            {
                var objData = new App_Detail_PLC05_KN_DA();
                DataSet dataSet = objData.GetByID(p_app_header_id, p_language_code);
                App_Detail_PLC05_KN_Info _App_Detail_Info = CBO<App_Detail_PLC05_KN_Info>.FillObjectFromDataSet(dataSet);
                ...
                return _App_Detail_Info;
            }
            catch (Exception ex)
            {
                Logger.LogException(ex);
                return new App_Detail_PLC05_KN_Info();
            }
        }
    }
}
